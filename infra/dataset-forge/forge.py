...
def cmd_annotate(a):
    ...
    out = []
    for idx, it in enumerate(man["items"]):
        try:
        ...
        for ti, y in enumerate(tile_origins(im.height)):
            ...
            arr = np.array(tile)
            r = det.predict(tile, imgsz=a.imgsz, conf=a.conf, verbose=False, device=a.device)[0]
            polys = []
            if r.boxes is not None and len(r.boxes):
                pred.set_image(arr)                      # heavy encode: once per TILE, not per box
                ...
                for m, sc in zip(masks.cpu().numpy(), scores.cpu().numpy()):
                    ...
                    if cv2.contourArea(c) < a.min_area:
                        continue
                    eps = a.simplify * cv2.arcLength(c, True)
                    ...
                    # Overlapping seed boxes routinely make SAM emit the SAME region twice;
                    # ultralytics then logs "duplicate labels removed" and silently drops one.
                    # Drop them here so the label file is right rather than merely tolerated.
