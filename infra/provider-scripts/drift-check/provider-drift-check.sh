...
while IFS= read -r dst; do
  ...
  if [ ! -f "$REPO_DIR/$rel" ]; then
    drifted=$((drifted + 1))
    emit warning "Deployed provider script has no repo source" \
