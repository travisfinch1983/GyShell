...
- [ ] **#6 GPU-fleet sparkline poll-rate setting** — Settings › General field for the GPU fleet sparkline refresh interval (allow down to 1s for rapid metrics). Fleet-panel data is cheap (gpuMonitor/Prometheus) so 1s is fine. (Service-card #4 sparklines share the nvtop 3s TTL — if rapid per-service is wanted too, lower the TTL w/ the on-view gating; cost caveat.)
- [ ] **Registry GPU-assignment drift (launch-registration bug)** — active-services registry records stale/wrong gpuPciIds: all 10 tts/rvc register 0000:8a:00.0 but their processes actually run on 8a/8b/8c/8d/b3. service-usage now trusts the exporter port-join over the registry (fixed), but the #1 service-card GPU BADGES still come from assignedGpus (registry gpuPciIds) and can be provably wrong. Fix at source: record the ACTUAL GPU a service lands on at launch/registration (or derive badges from the exporter like usage does).
