 ...
 */
/** Raised when a report saved but never reached its RAG collection. */
type NotifyFn = (e: { severity: 'warning' | 'error' | 'critical'; source: string; message: string; detail?: string }) => void

export function createReportsRouter(dataDir: string, notify?: NotifyFn): express.Router {
  ...
  router.put('/api/reports/:id', json, async (req: Req, res: Res) => {
    ...
    try {
      ...
      try {
        ...
        // Returning indexed:false covers an agent reading its own tool result and nobody
        // else. The degradation is otherwise invisible: the report is on screen and correct,
        // and the failure only shows up later as a search that finds nothing -- which is
        // indistinguishable from "this has never happened before", the wrong conclusion for
        // the one question reports exist to answer.
        notify?.({
          severity: 'warning', source: 'reports-rag',
          message: `Report saved but not indexed - it will not appear in search`,
          detail: `report ${id} in ${rt.collection}: ${indexError}`,
        })
      }
